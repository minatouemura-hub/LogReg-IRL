...
class Density_Ratio_Net(nn.Module):
    def __init__(self, state_space, device):
        ...
        self.conv1 = nn.Conv2d(
            in_channels=state_space, out_channels=32, kernel_size=2, stride=4, padding=1
        ).to(self.device)
        self.conv2 = nn.Conv2d(
            in_channels=32, out_channels=64, kernel_size=2, stride=2, padding=1
        ).to(self.device)
        self.conv3 = nn.Conv2d(
            in_channels=64, out_channels=64, kernel_size=1, stride=1, padding=1
        ).to(self.device)
        self.flatten = nn.Flatten(start_dim=1)
        self.fc1 = nn.Linear(576, 512).to(self.device)
